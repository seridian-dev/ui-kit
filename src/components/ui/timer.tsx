// Adapted from Cult UI's Timer (MIT, github.com/nolly-studio/cult-ui),
// verified against the real upstream source at
// https://raw.githubusercontent.com/nolly-studio/cult-ui/main/apps/www/registry/default/ui/timer.tsx
// (fetched directly, not reconstructed). Behavior/props are unchanged; adapted
// to this kit's conventions.
//
// Deviations from upstream: `data-slot` attributes added on the root, icon,
// and display parts; the default variant's layered shadow referenced upstream
// shadcn (Tailwind v3) channel vars via `hsl(var(--muted))` — remapped to this
// kit's Tailwind v4 token as `var(--color-muted)` (same visual intent; the raw
// `--muted` variable does not exist in this kit, which would have silently
// invalidated the shadow). No other colors required re-theming — upstream
// already uses theme tokens throughout.
"use client"
...
const timerVariants = cva(
  ...
  ],
  {
    variants: {
      variant: {
        default:
          "bg-background text-foreground border border-border shadow-[0_2px_4px_rgba(0,0,0,0.02),_0px_1px_2px_rgba(0,0,0,0.04)] shadow-[inset_0px_-2.10843px_0px_0px_var(--color-muted),_0px_1.20482px_6.3253px_0px_var(--color-muted)]",
        outline:
