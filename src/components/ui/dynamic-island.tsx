// Adapted from Cult UI's Dynamic Island (MIT, github.com/nolly-studio/cult-ui),
// verified against the real upstream source at
// https://raw.githubusercontent.com/nolly-studio/cult-ui/main/apps/www/registry/default/ui/dynamic-island.tsx
// (fetched directly, not reconstructed). Behavior/props are unchanged; adapted
// to this kit's conventions.
//
// Deviations from upstream: dropped the dual default export down to
// named-only (kit convention); `data-slot` attributes added on the island,
// container, content, and child parts. The hardcoded black surface
// (`bg-black`, `border-black/10`, `dark:border-white/5`,
// `focus-within:bg-neutral-900`, `text-black`) is kept verbatim — it is
// effect-intrinsic (the component emulates the iPhone's physical island
// hardware, which is black in both themes). Note: the content animates its
// `clipPath` to `url(#squircle-<size>)`, referencing SVG defs that the
// consumer must provide (upstream's demo does the same); without them the
// pill simply renders with its regular border radius.
"use client"
